  ...
  async function runAuto({ keep=false } = {}) {
    try {
      ...
      while (acceptedCount < targetAccepted && !cancel) {
        ...
        const pickSnapshot = (forceSnapshotTries > 0) || (Math.random() < baseUseRatio);
        ...
        if (ctxId) {
          try {
            ...
            if (ins && ins.ok && Array.isArray(ins.accepted) && ins.accepted.length > 0) { acceptedCount++; consecutiveMisses = 0; }
        ...
        if (tries % 3 === 0 && progressEl) progressEl.textContent = `Accepted ${acceptedCount}/${targetAccepted} (Created ${tries+1})`;
        // If we stall for many misses in a row, bias towards snapshot base briefly
        if (consecutiveMisses >= 40) { forceSnapshotTries = Math.max(forceSnapshotTries, 5); consecutiveMisses = 0; }
