...
export function draw(dto) {
  ...
  if (
    ...
    if (
      dto.player.attached &&
      typeof dto.player.entryDir === "number" &&
      dto.player.entryDir >= 0
    ) {
      ...
      const m = Math.max(1, Math.floor(tileSize * 0.1));
      const left = cellX + m,
        right = cellX + tileSize - m;
      const top = cellY + m,
        bottom = cellY + tileSize - m;
      const apexX = cx,
        apexY = cy;
...
function drawSlimPath(x, y, baseFill, dotFill) {
  ...
  const m = Math.max(2, Math.floor(s * 0.1)); // margin from corner
...
function drawTile(nameRaw, x, y) {
  ...
  if (n === "iceslimpath")
    return drawSlimPath(x, y, colors.iceBase, colors.wall);

  // Ice base
  if (n === "ice") return drawTileByFill(x, y, colors.iceBase);

  // Buttons (ring)
  if (n === "buttonallowexit")
    return drawButtonRing(x, y, colors.floor, colors.buttonGreen);
  if (n === "buttontoggle")
    return drawButtonRing(x, y, colors.floor, colors.buttonRed);

  // Default heuristic
  if (n.includes("hole")) return drawTileByFill(x, y, colors.hole);
  if (n.includes("wall") && !n.includes("fragile"))
    return drawTileByFill(x, y, colors.wall);
