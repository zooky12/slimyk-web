...
export function setupAutoLiteUI(api) {
  try { /* console.log('[auto-lite] setup start') */ } catch {}
  ...
  try {
    if (toggleBtn && panelEl && toggleBtn.dataset.bound !== "1") {
      ...
      toggleBtn.addEventListener("click", () => {
        ...
        if (next) {
          try { ensureMaskFromCurrentLevel(); renderMask(); requestAnimationFrame(() => { try { renderMask(); } catch {} }); } catch {}
        }
        ...
        try {
          ...
          if (isDefault) toggleBtn.textContent = next ? "Hide Auto Creator" : "Show Auto Creator";
        } catch {}
      });
      ...
      btn.addEventListener("click", (e) => {
        ...
        try {
          if (next && targetId === "autoMask") {
            ...
            renderMask();
            try { requestAnimationFrame(() => { try { renderMask(); } catch {} }); } catch {}
          }
        } catch {}
      });
    };
  ...
  function renderMask() {
    try {
      ...
      if (!maskCanvas || !maskState.grid) { dlog("[mask] render skipped: missing canvas or grid"); return; }
      const w = maskState.w,
        h = maskState.h;
      let ctx = null;
      try { ctx = maskCanvas.getContext("2d"); } catch(e) { /* ignore */ }
      dlog('[mask] ctx ok?', !!ctx);
      if (!ctx) { (console.warn || console.log) && (console.warn || console.log)("[mask] 2D context not available"); return; }
      ...
      dlog("[mask] render begin", { w, h, pw, cw: maskCanvas.width, ch: maskCanvas.height });
      ...
      try {
        ...
        for (const t of tiles) if (t && Number.isInteger(t.id)) tileIdToName[String(t.id)] = t.name;
      ...
      const isArrayLike = (v) =>
        Array.isArray(v) || (typeof ArrayBuffer !== "undefined" && ArrayBuffer.isView && ArrayBuffer.isView(v)) ||
        (!!v && typeof v.length === "number");
      try {
        const tiles = draw?.tiles;
        const tlen = tiles?.length | 0;
        const typed = (typeof ArrayBuffer !== "undefined" && ArrayBuffer.isView && ArrayBuffer.isView(tiles)) || false;
        const preview = tlen ? Array.from(tiles).slice(0, Math.min(24, tlen)) : [];
        const catalogSize = tileIdToName ? Object.keys(tileIdToName).length : 0;
        dlog('[mask] draw dto', { w: draw?.w, h: draw?.h, tlen, typed, preview, catalogSize });
        if (tlen) {
          const hist = {};
          for (let i = 0; i < tlen; i++) { const v = tiles[i] | 0; hist[v] = (hist[v] | 0) + 1; if (i > 4096) break; }
          const keys = Object.keys(hist).slice(0, 10);
          dlog('[mask] tile histogram', keys.reduce((o,k)=>{o[k]=hist[k];return o;},{}));
        }
      } catch {}
      ...
      function drawTileDetail(name, x, y, size) {
        try {
          ...
          if (n.includes("gril") || n.includes("grill")) {
            ...
            const prev = ctx.lineWidth; const prevStyle = ctx.strokeStyle;
            ctx.lineWidth = lw; ctx.strokeStyle = "rgba(230,233,255,0.35)";
            for (let i = step; i < s; i += step) {
              ctx.beginPath(); ctx.moveTo(px + i + 0.5, py); ctx.lineTo(px + i + 0.5, py + s); ctx.stroke();
              ctx.beginPath(); ctx.moveTo(px, py + i + 0.5); ctx.lineTo(px + s, py + i + 0.5); ctx.stroke();
            }
            ctx.lineWidth = prev; ctx.strokeStyle = prevStyle;
          }
          // Spikes: draw an X pattern
          else if (n.includes("spike")) {
            const lw = Math.max(2, Math.floor(s * 0.10));
            const prev = ctx.lineWidth; const prevStyle = ctx.strokeStyle;
            ctx.lineWidth = lw; ctx.strokeStyle = "#c0392b";
          ...
          else if (n.includes("button") || n.includes("plate") || n.includes("pressure")) {
            const r = Math.max(3, Math.floor(s * 0.28));
            const cx = px + s / 2, cy = py + s / 2;
            const fill = n.includes("red") ? "#e74c3c" : "#54d39b";
            ctx.beginPath(); ctx.fillStyle = fill; ctx.arc(cx, cy, r, 0, Math.PI * 2); ctx.fill();
          }
        } catch {}
      }
      const fallbackColorForId = (id) => {
        switch (id | 0) {
          case 1: return "#707070"; // wall
          case 2: return "#060606"; // hole
          case 16: return "#54d39b"; // exit-ish
          case 0: return "#fafafa"; // floor-ish
          default: return "#cccccc"; // generic
        }
      };
      ...
      if (
        ...
        try {
          if (gameCanvas && srcW > 0 && srcH > 0) {
            ...
            ctx.drawImage(gameCanvas, 0, 0, srcW, srcH, 0, 0, maskCanvas.width, maskCanvas.height);
          } else {
  ...
  function bindMaskPainting() {
    ...
    try {
      window.autoMaskDebug = {
        ensure: () => { try { ensureMaskFromCurrentLevel(); console.log('[mask] ensure called'); } catch(e) { console.warn('[mask] ensure error', e); } },
        render: () => { try { renderMask(); console.log('[mask] render called'); } catch(e) { console.warn('[mask] render error', e); } },
        info: () => ({ w: maskState.w, h: maskState.h, canvas: { w: maskCanvas?.width, h: maskCanvas?.height } })
      };
  ...
  function createChipRow(kind, name, label, defaults) {
    ...
    async function computeVisitedCellsFromMoves(levelDto, movesStr) {
      ...
      try {
        ...
        try {
          const dto0 = (typeof api.getState === 'function') ? api.getState(sid) : null;
          const px0 = dto0?.player?.x, py0 = dto0?.player?.y;
          if (Number.isInteger(px0) && Number.isInteger(py0) && py0 >= 0 && py0 < h && px0 >= 0 && px0 < w) visited[py0][px0] = true;
        } catch {}
        const charToDir = { w: 0, d: 1, s: 2, a: 3 };
        for (let i = 0; i < movesStr.length; i++) {
          const c = movesStr[i];
          ...
          const px = dto?.player?.x, py = dto?.player?.y;
          if (Number.isInteger(px) && Number.isInteger(py) && py >= 0 && py < h && px >= 0 && px < w) visited[py][px] = true;
          const step = r && r.step;
    ...
    if (addBucketBtn) {
      addBucketBtn.addEventListener("click", () => {
        ...
        if (typeof presetText === "string" && presetText.trim().length) rulesText = presetText;
        else if (presetText && typeof presetText === 'object') rulesText = JSON.stringify(presetText || {}, null, 2);
      ...
      try {
        ...
        try {
          ...
          if (String(e && e.message) === 'empty_allow_lists') {
            if (progressEl) progressEl.textContent = 'Select at least one tile or entity to place';
            if (runBtn) runBtn.disabled = false; if (stopBtn) stopBtn.disabled = true;
            return;
          }
          throw e;
        }
        const allowMovePlayerSetting = !!(settingsPreview && settingsPreview.mutation && settingsPreview.mutation.movePlayer);
        ...
        const tilesSel = Array.from(settingsPreview?.mutation?.tilesPlace || []);
        const entsSel = Array.from(settingsPreview?.mutation?.entitiesPlace || []);
        const cfg = {
        ...
        const targetAccepted = Number(document.getElementById("autoAttemptsCount")?.value) || 20;
        ...
        const frozenSelectSkew = Math.max(0, Number(settingsPreview?.selection?.skew) || 1);
        // Default to always using the original snapshot as base unless UI overrides it
        const frozenBaseUseRatio = Math.max(0, Math.min(1, Number(document.getElementById("autoBaseUseRatio")?.value ?? 1) || 1));
        const frozenGreedyRatio = Math.max(0, Math.min(1, Number(document.getElementById('autoGreedyRatio')?.value) || 0));
        const frozenRefreshEvery = Math.max(1, Number(document.getElementById("autoRefreshEvery")?.value) || 7);
        const frozenMask = (maskState && maskState.grid && maskState.h > 0 && maskState.w > 0)
          ? { w: maskState.w, h: maskState.h, grid: maskState.grid.map(r => r.slice()) }
          : null;
        ...
        while (acceptedCount < targetAccepted && !cancel) {
          ...
          if (!pickSnapshot && ctxId) {
            try {
              const sel = await (ald.aldSelectBaseCtx
                ? ald.aldSelectBaseCtx(ctxId, frozenSelectTopK, frozenSelectSkew)
                : api.aldSelectBaseCtx(ctxId, frozenSelectTopK, frozenSelectSkew));
          ...
          const cand = await buildCandidate(base, settingsPreview, cfg, nChanges, frozenGreedyRatio, allowMovePlayerSetting);
          ...
          if (cand && cand.rejected) {
            if (cand.reason === 'base_solution_still_works') {
              ...
              if (progressEl && i % 3 === 0) progressEl.textContent = `Discarded similar ${i+1}/${targetAccepted}`;
            } else if (cand.reason === 'solution_not_similar') {
              console.log('[auto] skip: solution not similar enough for replacement');
              if (progressEl) progressEl.textContent = `Skip (solution differs) ${i+1}/${targetAccepted}`;
            }
            continue;
          }
          let lvl = cand.lvl;
            ...
            try {
              ...
              try {
                ...
                for (let bi = 0; bi < src.length; bi++) {
                  ...
                  for (let fi = 0; fi < feats.length; fi++) {
                    ...
                    if (Number.isFinite(min) && Number.isFinite(max)) bands.push({ min, max });
                  }
                }
              } catch {}
              let okBand = true; // if no bands, don't block
              if (bands.length > 0 && fastest != null) {
                okBand = bands.some(b => fastest >= b.min && fastest <= b.max);
              }
              if (!okBand) {
                // Skip inserting this candidate locally
                consecutiveMisses++;
                if (progressEl && i % 3 === 0)
                  progressEl.textContent = `Discarded (fails band) ${i + 1}/${targetAccepted}`;
              ...
              try {
                const ins = await (ald.aldInsertCandidate
                  ? ald.aldInsertCandidate(ctxId, lvl, cfg)
                  : api.aldInsertCandidate(ctxId, lvl, cfg));
                if (ins && ins.ok && Array.isArray(ins.accepted) && ins.accepted.length > 0) {
                  acceptedCount++;
                  consecutiveMisses = 0;
                } else {
                  consecutiveMisses++;
          ...
          if (tries % 3 === 0 && progressEl)
            progressEl.textContent = `Accepted ${acceptedCount}/${targetAccepted} — Attempts ${tries + 1}`;
    ...
    if (genOneBtn)
      genOneBtn.addEventListener('click', async () => {
        try {
          ...
          if (!lvl || !lvl.tileGrid) {
            if (progressEl) progressEl.textContent = 'Generate one: no base level available';
            return;
          }
          const frozenGreedyRatio = Math.max(0, Math.min(1, Number(document.getElementById('autoGreedyRatio')?.value) || 0));
          const allowMovePlayerSetting = !!(settings && settings.mutation && settings.mutation.movePlayer);
          const baseChanges = Math.max(1, Number(document.getElementById('autoBaseChanges')?.value) || 1);
          const cfg = { depthCap: (settings?.solver?.DepthCap | 0) || 100, nodesCap: (settings?.solver?.NodesCap | 0) || 200000, timeCapSeconds: 5.0, enforceTimeCap: false };
          const cand = await buildCandidate(lvl, settings, cfg, baseChanges, frozenGreedyRatio, allowMovePlayerSetting);
          if (cand && cand.rejected) {
            if (cand.reason === 'base_solution_still_works') {
              if (progressEl) progressEl.textContent = 'Generate one: discarded (base solution still works)';
            } else if (cand.reason === 'solution_not_similar') {
              if (progressEl) progressEl.textContent = 'Generate one: skipped (solution differs)';
            } else {
              if (progressEl) progressEl.textContent = 'Generate one: no change (constraints/masks)';
            }
            return;
          }
          const out = cand.lvl;
        ...
        } catch(e){ console.warn('[auto] generate one failed', e); }
      });
    ...
    function importParamsFromObj(obj) {
      try {
        document
          .querySelectorAll("#autoTilesChips .tile-chip")
          .forEach((b) => { b.classList.remove("active"); b.setAttribute('aria-pressed','false'); });
        (obj.tiles?.allowed || []).forEach((nRaw) => {
          const n = String(nRaw || "");
          let btn = document.querySelector(`#autoTilesChips .tile-chip[data-tile="${n}"]`);
          if (!btn) {
            const lower = n.toLowerCase();
            btn = Array.from(document.querySelectorAll('#autoTilesChips .tile-chip')).find(b => String(b.dataset.tile||'').toLowerCase() === lower) || null;
          }
        ...
        (obj.entities?.allowed || []).forEach((nRaw) => {
          const n = String(nRaw || "");
          let btn = document.querySelector(`#autoEntitiesChips .tile-chip[data-entity="${n}"]`);
          if (!btn) {
            const lower = n.toLowerCase();
            btn = Array.from(document.querySelectorAll('#autoEntitiesChips .tile-chip')).find(b => String(b.dataset.entity||'').toLowerCase() === lower) || null;
          }
          if (btn) { btn.classList.add('active'); btn.setAttribute('aria-pressed','true'); }
        });
  ...
  function buildContextSettings() {
    function toBucketConfig(name, rulesText, presetName) {
      // Allow // comments in JSON textarea
      const text = typeof rulesText === 'string'
        ? rulesText.replace(/^\s*\/\/.*$/gm, '')
        : rulesText;
      // First try to parse the user-provided JSON
      try {
        const bc = JSON.parse(text);
        if (!bc.name) bc.name = name;
        if (bc.topK == null && name === "Hardest") bc.topK = 0;
        // If features are missing or empty, treat this as an invalid bucket config
        if (!Array.isArray(bc.features) || bc.features.length === 0) throw new Error('empty_features');
        return bc;
      } catch {}

      // If parsing failed, try falling back to the preset JSON (so notes don't disable rules)
      try {
        if (presetName && BUCKET_PRESETS && Object.prototype.hasOwnProperty.call(BUCKET_PRESETS, presetName)) {
      ...
      return { name, topK: name === "Hardest" ? 0 : 20, features: [
        ...
        { id: "solutionLength", mode: "Band", bandMin: 8, bandMax: 200, weight: 0, hard: true }
      ] };
    }
    ...
    try {
      if (Array.isArray(buckets) && buckets.length) {
        bucketsCfg = buckets.map((b) => toBucketConfig(b.name, b.rules, b.preset));
      } else {
        throw new Error('no_buckets');
      }
    } catch {
      const baseNames = ["Easy", "Medium", "Hard", "Hardest"];
      bucketsCfg = baseNames.map((name) => ({ name, topK: name === 'Hardest' ? 0 : 20, features: [] }));
    }
    const solver = {
    ...
    if ((!tilesSel || tilesSel.length === 0) && (!entsSel || entsSel.length === 0)) {
    ...
    let entityCounts = collectCounts("#autoEntitiesChips", "entity", new Set(["PlayerSpawn"]));
    ...
    try {
      if (Array.isArray(tilesSel) && tilesSel.length && tileCounts) {
        ...
        for (const k of Object.keys(tileCounts)) if (!allow.has(String(k))) delete tileCounts[k];
      }
      if (Array.isArray(entsSel) && entsSel.length && entityCounts) {
        const allow = new Set(entsSel.map(String));
        for (const k of Object.keys(entityCounts)) if (!allow.has(String(k))) delete entityCounts[k];
      }
    } catch {}
    ...
    try {
      if (maskState.grid && maskState.h > 0 && maskState.w > 0) {
        ...
        const h = snapshot?.height | 0, w = snapshot?.width | 0;
        editMaskTiles = Array.from({ length: h }, (_, y) => Array.from({ length: w }, (_, x) => (g[y]?.[x] | 0) === 2));
        editMaskEntities = Array.from({ length: h }, (_, y) => Array.from({ length: w }, (_, x) => (g[y]?.[x] | 0) >= 1));
      }
    } catch {}
    const allowMovePlayer = Array.isArray(entsSel) && entsSel.includes("PlayerSpawn");
    const mutation = {
      ...
      operatorWeights: { replaceTile: 0.15, placeEntity: 0.1, removeEntity: 0.05, greedyPlaceOne: 0.05, greedyRemoveOne: 0.05 },
    };
    const dedupe = {
      T_sol: 0.12, T_layout: 0.25, w_tiles: 0.4, w_entities: 0.4, w_spatial: 0.2,
    };
    const derived = (typeof DERIVED_DEFAULTS !== 'undefined' && Array.isArray(DERIVED_DEFAULTS)) ? DERIVED_DEFAULTS : [];
  ...
  async function buildCandidate(base, settingsPreview, cfg, nChanges, frozenGreedyRatio, allowMovePlayerSetting) {
    ...
    try {
      function computeReach(dto){
        try {
          const h=dto.height|0, w=dto.width|0, grid=dto.tileGrid||[];
          const inB=(x,y)=>x>=0&&y>=0&&x<w&&y<h;
          const isWall=(n)=> String(n||'').toLowerCase().includes('wall');
          // Only walls delimit the perimeter for player reach; everything else is passable for reach
          const pass=(x,y)=>{ const n=String(grid[y]?.[x]||'').toLowerCase(); return !isWall(n); };
          const seen=Array.from({length:h},()=>Array(w).fill(false));
          const q=[]; const ps=(dto.entities||[]).find(e=>e&&e.type==='PlayerSpawn');
          if(ps&&inB(ps.x,ps.y)&&pass(ps.x,ps.y)){ seen[ps.y][ps.x]=true; q.push({x:ps.x,y:ps.y}); }
          const dirs=[[1,0],[-1,0],[0,1],[0,-1]];
          while(q.length){ const {x,y}=q.shift(); for(const [dx,dy] of dirs){ const nx=x+dx,ny=y+dy; if(!inB(nx,ny)||seen[ny][nx]||!pass(nx,ny)) continue; seen[ny][nx]=true; q.push({x:nx,y:ny}); } }
          const reachPlusWalls=Array.from({length:h},()=>Array(w).fill(false));
          for(let y=0;y<h;y++) for(let x=0;x<w;x++){
            if(seen[y][x]) { reachPlusWalls[y][x]=true; continue; }
            if(isWall(grid[y]?.[x])){
              const adj=[[1,0],[-1,0],[0,1],[0,-1]].some(([dx,dy])=>{const nx=x+dx,ny=y+dy;return inB(nx,ny)&&seen[ny][nx];});
              if(adj) reachPlusWalls[y][x]=true;
            }
          }
          return { reach: seen, reachPlusWalls };
        } catch { return { reach:null, reachPlusWalls:null }; }
      }
      const reachInfo = computeReach(lvl);
      const h=(lvl?.height|0)||0, w=(lvl?.width|0)||0;
      ...
      try {
        if (maskState && maskState.grid && maskState.h>0 && maskState.w>0) {
          ...
          uiTileMask = Array.from({length:h},(_,y)=>Array.from({length:w},(_,x)=> (g[y]?.[x]|0) === 2));
          uiEntMask  = Array.from({length:h},(_,y)=>Array.from({length:w},(_,x)=> (g[y]?.[x]|0) >= 1));
        }
      } catch {}
      const effTile = Array.from({length:h},(_,y)=>Array.from({length:w},(_,x)=>{
        const allowUI = uiTileMask ? !!(uiTileMask[y]?.[x]) : true;
        const allowReach = !!(reachInfo.reach?.[y]?.[x] || reachInfo.reachPlusWalls?.[y]?.[x]);
        return allowUI && allowReach;
      }));
      const effEnt = Array.from({length:h},(_,y)=>Array.from({length:w},(_,x)=>{
    ...
    async function countLevel(dto){
      const tm = Object.create(null), em = Object.create(null);
      const H = dto.height|0, W = dto.width|0;
      for(let y=0;y<H;y++) for(let x=0;x<W;x++){ const n = String(dto.tileGrid?.[y]?.[x]||''); if(n) tm[n] = (tm[n]|0)+1; }
      for(const e of dto.entities||[]) if(e&&e.type){ const n=String(e.type); em[n]=(em[n]|0)+1; }
      return {tm, em};
    }
    function deficitsAndExcess(cur, wantTiles, wantEnts){
      const defT=[], defE=[], excT=[], excE=[];
      for(const [name, mm] of Object.entries(wantTiles||{})){
        const curN = cur.tm[name]|0;
        if(mm && mm.min!=null && curN < mm.min) defT.push([name, mm.min - curN]);
        if(mm && mm.max!=null && curN > mm.max) excT.push([name, curN - mm.max]);
      }
      for(const [name, mm] of Object.entries(wantEnts||{})){
        const curN = cur.em[name]|0;
        if(mm && mm.min!=null && curN < mm.min) defE.push([name, mm.min - curN]);
        if(mm && mm.max!=null && curN > mm.max) excE.push([name, curN - mm.max]);
      }
    ...
    function findTileDiff(before, after){
      try {
        const h = after.height|0, w = after.width|0;
        for(let y=0;y<h;y++){
          for(let x=0;x<w;x++){
            if ((before.tileGrid?.[y]?.[x]||'') !== (after.tileGrid?.[y]?.[x]||'')) return {x,y};
          }
        }
      } catch {}
      return null;
    ...
    function findEntityDelta(before, after, type){
      ...
      // look for added
      for(const [k, av] of a){ const bv = b.get(k)||0; if (av>bv){ const [xs,ys]=k.split(','); return {kind:'add', x: xs|0, y: ys|0}; } }
      // look for removed
      for(const [k, bv] of b){ const av = a.get(k)||0; if (bv>av){ const [xs,ys]=k.split(','); return {kind:'del', x: xs|0, y: ys|0}; } }
      // look for moved (one add + one del with same count) — already caught above as add/del
      return null;
    }
    function isAllowedTileCell(y,x){ return !!(settingsPreview?.mutation?.editAllowMask?.[y]?.[x]); }
    function isAllowedEntCell(y,x){ return !!(settingsPreview?.mutation?.editAllowEntitiesMask?.[y]?.[x]); }
    async function placeOneTile(dto, name){
      const opts = {
        tilesPlace: [name], entitiesPlace: [],
        ...
        editAllowEntitiesMask: settingsPreview?.mutation?.editAllowEntitiesMask || null,
      };
      const r = await (ald.aldPlaceOne ? ald.aldPlaceOne(dto, opts) : api.aldPlaceOne(dto, opts));
    ...
    async function placeOneEntity(dto, name){
      ...
      if (!delta) return dto;
      if (delta.kind === 'add' && !isAllowedEntCell(delta.y, delta.x)) return dto;
      return r.level;
    }
    async function removeOneEntity(dto, name){
      ...
      const r = await (ald.aldRemoveOne ? ald.aldRemoveOne(dto, opts) : api.aldRemoveOne(dto, opts));
      if (!(r && r.ok && r.level)) return dto;
      const delta = findEntityDelta(dto, r.level, name);
      if (!delta) return dto;
      if (delta.kind === 'del' && !isAllowedEntCell(delta.y, delta.x)) return dto;
      return r.level;
    }
    function replaceOneTileLocal(dto, overName, replNames){
      try{
        const H=dto.height|0,W=dto.width|0; const mask=settingsPreview?.mutation?.editAllowMask;
        for(let y=0;y<H;y++) for(let x=0;x<W;x++){
          if(dto.tileGrid?.[y]?.[x]===overName && (!mask || mask[y]?.[x])){
            const nn = replNames.find(n=>n && n!==overName);
            if(!nn) return dto;
            dto.tileGrid[y][x]=nn; return dto;
          }
        }
      }catch{}
      return dto;
    ...
    function fastPlaceOneTileLocal(dto, name){
      try {
        ...
        const cells=[]; for(let y=0;y<H;y++) for(let x=0;x<W;x++) if(!mask || mask[y]?.[x]){ if(String(dto.tileGrid?.[y]?.[x]||'')!==name) cells.push({x,y}); }
    ...
    const allowedTiles = Array.from(settingsPreview?.mutation?.tilesPlace||[]);
    let usedCountEdits = 0;
    let safety=40;
    while(safety-- > 0){
      const cur = await countLevel(lvl);
      const {defT,defE,excT,excE} = deficitsAndExcess(cur, wantTiles, wantEnts);
      if(defT.length===0 && defE.length===0 && excT.length===0 && excE.length===0) break;
      if(defT.length){
        const [name] = defT[0];
        // Try fast local placement first (instant)
        const beforeGrid = JSON.stringify(lvl.tileGrid);
        lvl = fastPlaceOneTileLocal(lvl, name);
        if (JSON.stringify(lvl.tileGrid) !== beforeGrid) { usedCountEdits++; continue; }
        // Fallback to engine op
        const before = lvl; lvl = await placeOneTile(lvl, name); if (lvl !== before) usedCountEdits++; continue;
      }
      if(defE.length){ const [name] = defE[0]; const before = lvl; lvl = await placeOneEntity(lvl, name); if (lvl !== before) usedCountEdits++; continue; }
      if(excE.length){ const [name] = excE[0]; const before = lvl; lvl = await removeOneEntity(lvl, name); if (lvl !== before) usedCountEdits++; continue; }
      if(excT.length){ const [name] = excT[0]; const cur2 = await countLevel(lvl); const {defT:dt} = deficitsAndExcess(cur2, wantTiles, wantEnts); const preferred = dt.length ? [dt[0][0]] : allowedTiles.filter(n=>n!==name); const before = JSON.stringify(lvl.tileGrid); lvl = replaceOneTileLocal(lvl, name, preferred); if (JSON.stringify(lvl.tileGrid) !== before) usedCountEdits++; continue; }
    }
    // exploration helpers
    function cellsFromMask(mask){ const out=[]; const H = lvl.height|0, W = lvl.width|0; for(let y=0;y<H;y++) for(let x=0;x<W;x++) if(mask?.[y]?.[x]) out.push({x,y}); for(let i=out.length-1;i>0;i--){ const j=(Math.random()*(i+1))|0; const t=out[i]; out[i]=out[j]; out[j]=t; } return out; }
    function selectCellsWithPathBias(mask, pathMask){ const all=cellsFromMask(mask); if(!pathMask||!all.length) return all; const onPath=[]; for(const c of all){ if(pathMask?.[c.y]?.[c.x]) onPath.push(c);} if(onPath.length && Math.random()<0.8) return onPath; return all; }
    function okCountsTile(changeFrom, changeTo, tm, tCounts){ const mmFrom=tCounts&&tCounts[changeFrom]; const mmTo=tCounts&&tCounts[changeTo]; const curFrom=tm[changeFrom]|0, curTo=tm[changeTo]|0; if(mmFrom&&mmFrom.min!=null&&(curFrom-1)<mmFrom.min) return false; if(mmTo&&mmTo.max!=null&&(curTo+1)>mmTo.max) return false; return true; }
    function okCountsEntityAdd(name, em, eCounts){ const mm=eCounts&&eCounts[name]; const cur=em[name]|0; if(mm&&mm.max!=null&&(cur+1)>mm.max) return false; return true; }
    function okCountsEntityRemove(name, em, eCounts){ const mm=eCounts&&eCounts[name]; const cur=em[name]|0; if(mm&&mm.min!=null&&(cur-1)<mm.min) return false; return true; }
    function cellOccupied(dto,x,y){ return Array.isArray(dto.entities) && dto.entities.some(e=>e&&e.x===x&&e.y===y); }
    async function exploreOnce(){
      ...
      const tryTile = async () => {
        ...
        let tries=20; while(tries-- > 0){ const c=cells[(Math.random()*cells.length)|0]; if(!c) break; const old=String(lvl.tileGrid?.[c.y]?.[c.x]||''); const pick=tList[(Math.random()*tList.length)|0]; if(!pick||pick===old) continue; if(!okCountsTile(old,pick,countsNow.tm, settingsPreview?.mutation?.tileCounts)) continue; lvl.tileGrid[c.y][c.x]=pick; countsNow.tm[old]=(countsNow.tm[old]|0)-1; countsNow.tm[pick]=(countsNow.tm[pick]|0)+1; return true; }
        return false;
      };
      const tryEntity = async () => {
        const mask = settingsPreview?.mutation?.editAllowEntitiesMask || [];
        const cells = selectCellsWithPathBias(mask, pathMask);
        const eName = eList[(Math.random()*eList.length)|0]; if(!eName) return false;
        const op = ['delete','place','move'][(Math.random()*3)|0];
        if (op==='delete'){
          if (!okCountsEntityRemove(eName, countsNow.em, settingsPreview?.mutation?.entityCounts)) return false;
          const idx = (lvl.entities||[]).findIndex(e=>e&&e.type===eName); if (idx<0) return false;
          const e = lvl.entities[idx]; if(!mask?.[e.y]?.[e.x]) return false;
          lvl.entities.splice(idx,1); countsNow.em[eName]=(countsNow.em[eName]|0)-1; return true;
        }
        if (op==='place'){
          if (!okCountsEntityAdd(eName, countsNow.em, settingsPreview?.mutation?.entityCounts)) return false;
          let tries=20; while(tries-- > 0){ const c=cells[(Math.random()*cells.length)|0]; if(!c) break; if (cellOccupied(lvl,c.x,c.y)) continue; if (eName==='PlayerSpawn' && !settingsPreview?.mutation?.movePlayer) return false; (lvl.entities||(lvl.entities=[])).push({ type:eName, x:c.x, y:c.y }); countsNow.em[eName]=(countsNow.em[eName]|0)+1; return true; }
          return false;
        }
        const idx = (lvl.entities||[]).findIndex(e=>e&&e.type===eName); if (idx<0) return false;
        if (eName==='PlayerSpawn' && !settingsPreview?.mutation?.movePlayer) return false;
        // Moving touches old (delete) and new (place); require both to be in allowed mask
        const e0 = lvl.entities[idx]; if (!mask?.[e0.y]?.[e0.x]) return false;
        let tries=20; while(tries-- > 0){ const c=cells[(Math.random()*cells.length)|0]; if(!c) break; if (cellOccupied(lvl,c.x,c.y)) continue; const e=lvl.entities[idx]; e.x=c.x; e.y=c.y; return true; }
        return false;
      };
      if (chooseTile) {
    ...
    if (exploreSteps > 0 && !pathMask) {
      ...
      try { baseMoves = await getShortestMovesForBase(base, cfg); } catch {}
      try { if (baseMoves) pathMask = await computeVisitedCellsFromMoves(base, baseMoves); } catch {}
    }
    while (exploreSteps-- > 0) { const ok = await exploreOnce(); if (!ok) break; }
    // Enforce PlayerSpawn immobility if not allowed
    try {
      if (!allowMovePlayerSetting && lvl && base) {
        const findPs = (d) => (Array.isArray(d.entities) ? d.entities.find(e => e && e.type === "PlayerSpawn") : null);
        const psBase = findPs(base);
        if (psBase) {
          if (!Array.isArray(lvl.entities)) lvl.entities = [];
          const idx = lvl.entities.findIndex(e => e && e.type === "PlayerSpawn");
          const fixed = { type: "PlayerSpawn", x: psBase.x|0, y: psBase.y|0 };
          if (idx >= 0) lvl.entities[idx] = fixed; else lvl.entities.push(fixed);
        }
      }
    } catch {}
    ...
    try {
      if (Math.random() < Math.max(0, Math.min(1, frozenGreedyRatio))) {
        ...
        const res = await (ald.aldPlaceOne ? ald.aldPlaceOne(lvl, opts) : api.aldPlaceOne(lvl, opts));
        if (res && res.ok && res.level) {
          ...
          if (tdiff) {
            ...
            const posCount = (dto)=>{
              const m=new Map(); for(const e of (dto.entities||[])){ if(!e) continue; const k=e.x+","+e.y; m.set(k,(m.get(k)||0)+1);} return m; };
            const b=posCount(before), a=posCount(res.level);
            let ok=false;
            for (const [k,av] of a){ const bv=b.get(k)||0; if(av>bv){ const [xs,ys]=k.split(','); if(isAllowedEntCell(ys|0, xs|0)) { ok=true; break; } else { ok=false; break; } } }
            if (!ok){ for (const [k,bv] of b){ const av=a.get(k)||0; if(bv>av){ const [xs,ys]=k.split(','); if(isAllowedEntCell(ys|0, xs|0)) { ok=true; break; } else { ok=false; break; } } } }
            if (ok) lvl = res.level;
          }
        }
      }
    ...
    if (!baseMoves) { try { baseMoves = await getShortestMovesForBase(base, cfg); } catch {} }
    if (baseMoves) {
      try {
        ...
        if (topC && topC.length && topC.movesPacked) {
          ...
          if (dist > 3) return { rejected: true, reason: 'solution_not_similar' };
        }
      } catch {}
    }
    ...
    try {
      ...
      if (baseMoves) {
        ...
        if (works) return { rejected: true, reason: 'base_solution_still_works' };
      }
    } catch {}
