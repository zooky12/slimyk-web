...
export async function initWasm(baseUrl) {
  ...
  __wasmBooting = (async () => {
      ...
      .withModuleConfig({
        ...
        loadBootResource: (type, name, defaultUri /*, integrity*/) => {
          ...
          if (
            ...
            String(name).endsWith("/blazor.boot.json")
          ) {
            return (async () => {
              const res = await fetch(defaultUri, { cache: "no-store" });
              const boot = await res.json();

              // Iterate all known resource groups and strip any *.symbols
              const groups = [
                "resources",
                "cacheBootResources",
                "debugBuild",
                "assets",
                "extensions", // defensive: some versions stash under different keys
              ];
              const stripSymbols = (obj) => {
                ...
                for (const k of Object.keys(obj)) {
                  ...
                  if (v && typeof v === "object") {
                    // Nested resource dictionaries (e.g., obj.asm, obj.jsModuleNative, obj.wasmSymbols, etc.)
                    if (!Array.isArray(v)) stripSymbols(v);
                  }
                }
                // Remove any key whose name contains '.symbols'
                for (const k of Object.keys(obj)) {
                  if (k.includes(".symbols")) delete obj[k];
                }
              };
              ...
              if (
                boot &&
                boot.resources &&
                typeof boot.resources === "object"
              ) {
                stripSymbols(boot.resources);
              }
              // Also, some SDKs carry a top-level "resources" arrays/maps with hash metadata
              // This pass cleans any stray symbol entries anywhere in the tree.
              stripSymbols(boot);

              // Optional: some SDKs carry integrity/hashes for js native module;
              // if a leftover entry references .symbols' integrity, remove that property.
              const cleanseIntegrity = (node) => {
              ...
              cleanseIntegrity(boot);

              const blob = new Blob([JSON.stringify(boot)], {
    ...
    function addTypes(obj) {
      for (const k of Object.keys(obj || {})) typeNames.add(k);
    }
    ...
    if (!E || typeof E.Engine_Init !== "function") {
      for (const k of Object.keys(ex || {})) {
        const t = ex[k];
        if (t && typeof t.Engine_Init === "function") {
          E = t;
          break;
        }
      }
    ...
    if (!E || typeof E.Engine_Init !== "function") {
      ...
      const binder =
        ...
        (runtime.BINDING ||
          runtime.binding ||
          (runtime.mono_bind_static_method && {
            bind_static_method: runtime.mono_bind_static_method,
          }));
      if (binder && typeof binder.bind_static_method === "function") {
        const asm = (cfg?.mainAssemblyName || "EngineWasm").replace(
          /\.dll$/i,
          ""
        );
    ...
    if (!E || typeof E.Engine_Init !== "function") {
      ...
      console.error("[wasm-adapter] Unable to locate Engine_Init export", {
        mainAssemblyName: cfg?.mainAssemblyName,
        tried: Array.from(tried),
        availableTypes: all,
      });
      throw new Error(
        "WASM exports not found (Engine_Init). Available export types: " +
          all.join(", ")
      );
    }
    const has = (name) => E && typeof E[name] === "function";
    const asmName = (cfg?.mainAssemblyName || "EngineWasm").replace(
      /\.dll$/i,
      ""
    );
      ...
      try {
        ...
        if (binder && typeof binder.bind_static_method === "function") {
          ...
          const sigMap = {
            ...
            Level_ApplyEdit:
              "(System.String,System.Int32,System.Int32,System.Int32,System.Int32,System.Int32)",
            Level_SetTile:
              "(System.String,System.Int32,System.Int32,System.Int32)",
            Level_SpawnEntity:
              "(System.String,System.Int32,System.Int32,System.Int32)",
            Level_RemoveEntityAt: "(System.String,System.Int32,System.Int32)",
            Level_SetEntityOrientation:
              "(System.String,System.Int32,System.Int32)",
            Level_SetPlayer: "(System.String,System.Int32,System.Int32)",
          };
          if (sigMap[name])
            candidates.push(`[${asmName}] Exports:${name}${sigMap[name]}`);
          try {
            for (const t of Object.keys(ex || {})) {
              candidates.push(`[${asmName}] ${t}:${name}`);
            }
          } catch {}
          for (const sig of candidates) {
            try {
              console.debug &&
                console.debug("[wasm-adapter] late-bind try", sig);
            } catch {}
            try {
    ...
    const api = {
      ...
      // Builder ops
      setTile: (sid, x, y, tileId) =>
        JSON.parse(E.Level_SetTile(sid, x, y, tileId)),
      spawnEntity: (sid, typeId, x, y) =>
        JSON.parse(E.Level_SpawnEntity(sid, typeId, x, y)),
      removeEntityAt: (sid, x, y) =>
        JSON.parse(E.Level_RemoveEntityAt(sid, x, y)),
      setEntityOrientation: (sid, entityId, rot) =>
        JSON.parse(E.Level_SetEntityOrientation(sid, entityId, rot)),
      setPlayer: (sid, x, y) => JSON.parse(E.Level_SetPlayer(sid, x, y)),
      applyEdit: (sid, kind, x, y, type, rot) =>
        JSON.parse(E.Level_ApplyEdit(sid, kind, x, y, type, rot)),
      resize: async (sid, add, dir) => {
        const fn = has("Level_Resize")
          ? E.Level_Resize
          : await ensureBound("Level_Resize");
      ...
      rotateEntityAt: (sid, x, y, rot) =>
        JSON.parse(E.Level_ApplyEdit(sid, 4, x, y, -1, rot)),

      // Optional features (Unity-Editor-only in this repo)
      solverAnalyze: async (level, cfg) => {
        let fn = has("Solver_Analyze")
          ? E.Solver_Analyze
          : await ensureBound("Solver_Analyze");
      ...
      aldTryMutate: async (level) => {
        let fn = has("ALD_TryMutate")
          ? E.ALD_TryMutate
          : await ensureBound("ALD_TryMutate");
      ...
      aldInsertCandidate: async (ctxId, level, solverCfg) => {
        let fn = has("ALD_InsertCandidate")
          ? E.ALD_InsertCandidate
          : await ensureBound("ALD_InsertCandidate");
        if (!fn) throw new Error("ALD_InsertCandidate not available");
        return JSON.parse(
          fn(
            String(ctxId || ""),
            toJsonString(level),
            solverCfg ? JSON.stringify(solverCfg) : null
          )
        );
      },
      aldGetBucketsSummary: async (ctxId) => {
        let fn = has("ALD_GetBucketsSummary")
          ? E.ALD_GetBucketsSummary
          : await ensureBound("ALD_GetBucketsSummary");
        if (!fn) throw new Error("ALD_GetBucketsSummary not available");
        return JSON.parse(fn(String(ctxId || "")));
      },
      aldSelectBaseCtx: async (ctxId, topK, skew) => {
        let fn = has("ALD_SelectBaseCtx")
          ? E.ALD_SelectBaseCtx
          : await ensureBound("ALD_SelectBaseCtx");
      ...
      aldMutate: async (ctxId, level, mutate) => {
        let fn = has("ALD_Mutate")
          ? E.ALD_Mutate
          : await ensureBound("ALD_Mutate");
        if (!fn) throw new Error("ALD_Mutate not available");
        return JSON.parse(
          fn(
            String(ctxId || ""),
            toJsonString(level),
            mutate ? JSON.stringify(mutate) : null
          )
        );
      },
      aldSelectBase: async (entries, topK, skew) => {
        let fn = has("ALD_SelectBase")
          ? E.ALD_SelectBase
          : await ensureBound("ALD_SelectBase");
      ...
      aldSolutionSimilarityMoves: async (movesA, movesB) => {
        let fn = has("ALD_SolutionSimilarityMoves")
          ? E.ALD_SolutionSimilarityMoves
          : await ensureBound("ALD_SolutionSimilarityMoves");
        if (!fn)
          throw new Error(
            "ALD_SolutionSimilarityMoves not available in this build"
          );
        return Number(fn(String(movesA || ""), String(movesB || "")));
      },
      aldLayoutSimilarity: async (levelA, levelB) => {
        let fn = has("ALD_LayoutSimilarity")
          ? E.ALD_LayoutSimilarity
          : await ensureBound("ALD_LayoutSimilarity");
        if (!fn)
          throw new Error("ALD_LayoutSimilarity not available in this build");
        return Number(fn(toJsonString(levelA), toJsonString(levelB)));
      },
      aldPlaceOne: async (level, opts) => {
        let fn = has("ALD_PlaceOne")
          ? E.ALD_PlaceOne
          : await ensureBound("ALD_PlaceOne");
        if (!fn) throw new Error("ALD_PlaceOne not available in this build");
        return JSON.parse(
          fn(toJsonString(level), opts ? JSON.stringify(opts) : null)
        );
      },
      aldRemoveOne: async (level, opts) => {
        let fn = has("ALD_RemoveOne")
          ? E.ALD_RemoveOne
          : await ensureBound("ALD_RemoveOne");
      ...
      __debugExports: async () => {
        ...
        let solAvail = false;
        try {
          let ping = has("Debug_Ping")
            ? E.Debug_Ping
            : await ensureBound("Debug_Ping");
          if (ping) pingOk = !!ping();
        } catch {}
        try {
          let av = has("Solver_ExportsPresent")
            ? E.Solver_ExportsPresent
            : await ensureBound("Solver_ExportsPresent");
          if (av) solAvail = !!av();
        } catch {}
        return {
          asm: asmName,
