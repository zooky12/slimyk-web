...
namespace SlimeGrid.Tools.ALD
{
    ...
    public static class GreedyOps
    {
        public sealed class PlaceOneOptions
        {
            ...
            // Optional starting min-steps threshold.
            public int minSteps { get; set; } = 0;
            // If true, raise minSteps to base fastest solution length.
            public bool requireHarderThanBase { get; set; } = false;
        }
        ...
        static bool MaskAllows(bool[][] M, int x, int y)
        {
        ...
        public static GreedyResult PlaceOne(LevelDTO seedDto, PlaceOneOptions opts)
        {
            ...
            // Compute base fastest solution (if any) and store moves for quick replay checks
            int currentMinSteps = Math.Max(0, opts.minSteps);
            SlimeGrid.Tools.Solver.PackedMoves baseMoves = default;
            try
            {
                var baseRep = BruteForceSolver.AnalyzeBfs(seed, new SolverConfig { DepthCap = opts.maxDepth, NodesCap = opts.maxNodes, MinSteps = 0 });
                if (baseRep.topSolutions != null && baseRep.topSolutions.Count > 0)
                {
                    if (opts.requireHarderThanBase)
                        currentMinSteps = Math.Max(currentMinSteps, baseRep.topSolutions[0].length);
                    baseMoves = new SlimeGrid.Tools.Solver.PackedMoves { Buffer = baseRep.topSolutions[0].movesPacked, Length = baseRep.topSolutions[0].length };
                }
            } catch { }

            // Quick replay: try the base shortest path against a modified candidate; if it still solves, discard candidate
            static bool BasePathStillSolves(LevelDTO dto, SlimeGrid.Tools.Solver.PackedMoves pm)
            {
                try
                {
                    ...
                    // map 0..3 -> N,E,S,W
                    SlimeGrid.Logic.Dir Map(byte code)
                    {
                        return code switch { 0 => SlimeGrid.Logic.Dir.N, 1 => SlimeGrid.Logic.Dir.E, 2 => SlimeGrid.Logic.Dir.S, 3 => SlimeGrid.Logic.Dir.W, _ => SlimeGrid.Logic.Dir.N };
                    }
                    for (int i = 0; i < pm.Length; i++)
                    {
                        var dir = Map(pm.GetAt(i));
                        var res = SlimeGrid.Logic.Engine.Step(s, dir);
                        if (res.GameOver) return false;
            ...
            if (tilesAllowed.Count > 0)
            {
                ...
                for (int x = 0; x < W; x++)
                {
                    // Respect tile edit mask early
                    if (!MaskAllows(opts.editAllowMask, x, y)) continue;
                    foreach (var name in tilesAllowed)
                    {
                        ...
                        // If base path still solves, discard early
                        if (baseMoves.Length > 0 && BasePathStillSolves(testDto, baseMoves)) continue;
                        // Configure dynamic min-steps and stop-on-improvement
                        var cfgLocal = new SolverConfig { DepthCap = opts.maxDepth, NodesCap = opts.maxNodes, MinSteps = currentMinSteps, StopOnFirstLongerThanMin = (opts.requireHarderThanBase && currentMinSteps > 0) };
                        var report = BruteForceSolver.AnalyzeBfs(Loader.FromDTO(testDto), cfgLocal);
                        if (report.solvedTag == "too_short" || report.topSolutions == null || report.topSolutions.Count == 0) continue;
                        ...
                        if (Better(fastest, deads, sols))
                        {
                            ...
                            if (opts.requireHarderThanBase && fastest > currentMinSteps) currentMinSteps = fastest;
                        }
                    }
                }
            }
            ...
            if (entsAllowed.Count > 0)
            {
                ...
                for (int x = 0; x < W; x++)
                {
                    if (!MaskAllows(opts.editAllowEntitiesMask, x, y)) continue;
                    foreach (var name in entsAllowed)
                    {
                        ...
                        // If base path still solves, discard early
                        if (baseMoves.Length > 0 && BasePathStillSolves(testDto2, baseMoves)) continue;
                        var cfgLocal2 = new SolverConfig { DepthCap = opts.maxDepth, NodesCap = opts.maxNodes, MinSteps = currentMinSteps, StopOnFirstLongerThanMin = (opts.requireHarderThanBase && currentMinSteps > 0) };
                        var report2 = BruteForceSolver.AnalyzeBfs(Loader.FromDTO(testDto2), cfgLocal2);
                        if (report2.solvedTag == "too_short" || report2.topSolutions == null || report2.topSolutions.Count == 0) continue;
                        ...
                        if (Better(fastest2, deads2, sols2))
                        {
                            ...
                            if (opts.requireHarderThanBase && fastest2 > currentMinSteps) currentMinSteps = fastest2;
                        }
                    }
                }
            }
        ...
        public static GreedyResult RemoveOne(LevelDTO seedDto, RemoveOneOptions opts)
        {
            ...
            for (int i = 0; i < stateEntities.Count; i++)
            {
                ...
                if (!MaskAllows(opts.editAllowEntitiesMask, e.x, e.y)) continue;
